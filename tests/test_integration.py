...
class TestUserJourney:
    def test_register_login_create_redirect_list_update_delete(self, client):
        reg = client.post(
            "/api/auth/register",
            json={
        ...
        listing = client.get("/api/links/my", headers=headers)
        ...
        after_delete = client.get(f"/{short_code}", follow_redirects=False)
        assert after_delete.status_code == 404

        empty_list = client.get("/api/links/my", headers=headers)
        assert empty_list.status_code == 200
        assert not any(link["short_code"] == short_code for link in empty_list.json())

    def test_refresh_after_login(self, client, test_user):
        login = client.post(
            "/api/auth/login",
